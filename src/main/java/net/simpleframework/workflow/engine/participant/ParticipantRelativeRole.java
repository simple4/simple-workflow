...
public class ParticipantRelativeRole extends AbstractParticipants {
	...
	public Collection<Participant> participants(final IScriptEval script, final KVMap variables) {
		...
		if (rType == ERelativeType.processInitiator) {
			final ProcessBean process = getModuleContext().getActivityMgr().getProcessBean(
					activityComplete.getActivity());
			final ID userId = process.getUserId();
			if (userId != null) {
				if (StringUtils.hasText(relative)) {
					final Collection<Participant> _participants = getModuleContext().getParticipantMgr()
			...
			if (preActivity != null) {
				...
				if (StringUtils.hasText(relative)) {
					...
					// 其它已完成任务项
					final IDataQuery<WorkitemBean> qs = getModuleContext().getWorkitemMgr()
							.getWorkitemList(preActivity, EWorkitemStatus.complete);
